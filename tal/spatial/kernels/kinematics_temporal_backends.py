...
from .kinematics_temporal_kernels import cumulative_simpson_kernel, cumulative_trapezoid_kernel
...
def cumulative_simpson_block_backend(
    values: np.ndarray,
    param: np.ndarray,
    valid: np.ndarray,
    *,
    ...
    owner = "spatial.kinematics.temporal.integral_backend"
    if backend == KINEMATICS_TEMPORAL_BACKEND_NUMPY:
        return cumulative_simpson_kernel(values, param, valid, initial_value=initial_value)
    if backend == KINEMATICS_TEMPORAL_BACKEND_NUMBA:
        from .kinematics_temporal_numba_backends import cumulative_simpson_block_numba

        return cumulative_simpson_block_numba(values, param, valid, initial_value=initial_value, owner=owner)
    raise ValueError(f"{owner}: unsupported temporal backend {backend!r}.")
