...
def test_combine_assemble_001_dense_nd_layout_shape_and_core_labels() -> None:
    ...
    assert _roles(out.as_dataset(copy="none"))[3] == ("row", "col", "axis")
    assert out.as_dataset(copy="none").coords["row"].values.tolist() == ["r0", "r1"]
    assert out.as_dataset(copy="none").coords["col"].values.tolist() == ["c0", "c1"]
    col_axis = xr.DataArray(["c0", "c1"], dims=("col",), coords={"col": ["c0", "c1"]})
    row_axis = xr.DataArray(["r0", "r1"], dims=("row",), coords={"row": ["r0", "r1"]})
    top = xr.concat([a.as_dataset(copy="none")["x"], b.as_dataset(copy="none")["x"]], dim=col_axis)
    bottom = xr.concat([c.as_dataset(copy="none")["x"], d.as_dataset(copy="none")["x"]], dim=col_axis)
    expected = xr.concat([top, bottom], dim=row_axis)
    xr.testing.assert_allclose(out.as_dataset(copy="none")["x"], expected)
    assert out.as_dataset(copy="none").attrs["tal"]["core"]["param_coord"] == {"name": "tau"}
    assert out.as_dataset(copy="none").attrs["tal"]["core"]["validity"]["sequence_size_coord"] == "sample_size"
...
def test_combine_assemble_003_exact_alignment_label_safe_not_positional() -> None:
    ...
    a = _vector_leaf(offset=0.0)
    b_ds = _vector_leaf(offset=5.0).as_dataset(copy="none").transpose("trial", "sample", "axis")
    ...
    expected = xr.concat([a.as_dataset(copy="none")["x"], b.as_dataset(copy="none")["x"]], dim=row_axis)
    xr.testing.assert_allclose(out.as_dataset(copy="none")["x"], expected)
...
def test_combine_assemble_004_shared_leaf_core_dims_preserved() -> None:
    ...
    assert _roles(out.as_dataset(copy="none"))[3] == ("block", "lr", "lc")
