...
def test_reduce_core_p9c_001_ao_reducer_surface_restored_with_deterministic_dim_semantics() -> None:
    ...
    assert set(out.unsafe_data.data_vars) == {"signal", "energy"}
    assert out.unsafe_data["signal"].dims == ()
    assert out.unsafe_data["energy"].dims == ()
...
def test_reduce_core_p9c_001b_reducer_preserves_declared_roles_when_sequence_dim_removed() -> None:
    ...
    assert core_dims == ()
    assert read_param_coord_name(out.unsafe_data) is None
...
def test_reduce_core_p9c_006_weight_forms_and_exact_alignment_rules_are_enforced() -> None:
    ...
    out = ao.mean(dim="sample", weights=w_nd, validate=True).unsafe_data
    ...
    out_map = ao.sum(dim=("trial", "sample"), weights=w_map, validate=True).unsafe_data
...
def test_reduce_core_p9c_003_weighted_mean_is_explicit_and_not_silently_ignored() -> None:
    ...
    ao = _ao_numeric_bool_object()
    unweighted = ao.mean(dim="sample", validate=True).unsafe_data["energy"].to_numpy()
    weighted = ao.mean(dim="sample", weights=np.array([1.0, 2.0, 1.0], dtype=float), validate=True).unsafe_data[
        "energy"
    ].to_numpy()
...
def test_reduce_core_p9c_005_dim_none_reduces_over_reducible_non_component_dims() -> None:
    ...
    ao = _ao_numeric_bool_object()
    out_default = ao.sum(validate=True).unsafe_data
    assert out_default["signal"].dims == ()
    assert out_default["energy"].dims == ()

    out_explicit = ao.sum(dim="sample", validate=True).unsafe_data
