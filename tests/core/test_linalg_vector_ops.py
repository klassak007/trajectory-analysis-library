...
def test_linalg_vector_001_dot_norm_core_dim_semantics() -> None:
    ...
    out_norm = norm(left)
    assert _core_dims(out_dot.unsafe_data) == ()
    assert _core_dims(out_norm.unsafe_data) == ()
    assert "datavar" in out_dot.unsafe_data.data_vars
    assert "datavar" in out_norm.unsafe_data.data_vars
    assert not any("_dot_" in name for name in out_dot.unsafe_data.data_vars)
    assert not any("_norm" in name for name in out_norm.unsafe_data.data_vars)
...
def test_linalg_core_017_vector_dot_semantics_values_parity() -> None:
    ...
    expected = xr.dot(left.unsafe_data["x"], right.unsafe_data["x"], dim=["axis"]).rename("datavar")
...
def test_linalg_core_018_vector_norm_semantics_values_parity() -> None:
    ...
    expected = _expected_norm(left.unsafe_data["x"], axis="axis", ord=2)
    xr.testing.assert_allclose(out.unsafe_data["datavar"], expected)
    xr.testing.assert_identical(out.unsafe_data, out_method.unsafe_data)
...
def test_linalg_hard_051_dot_plain_ao_inputs_fallback_to_array() -> None:
    ...
    out_ds = dot(left_ao.unsafe_data, right_ao.unsafe_data)
...
def test_linalg_hard_052_norm_plain_ao_inputs_fallback_to_array() -> None:
    ...
    left_ao = _vector_ao(np.arange(12, dtype=float).reshape(2, 2, 3), axis="axis")
    out = norm(left_ao)
    out_ds = norm(left_ao.unsafe_data)
    assert type(out) is Array
    assert type(out_ds) is Array
...
def test_linalg_hard_054_norm_builtin_wrapper_type_routing_preserves_values() -> None:
    ...
    left = Vector(_vector_ao(np.arange(12, dtype=float).reshape(2, 2, 3), axis="axis"))
    out = norm(left, ord=1)
    expected = _expected_norm(left.unsafe_data["x"], axis="axis", ord=1)
    assert type(out) is Array
    xr.testing.assert_allclose(out.unsafe_data["datavar"], expected)
...
def test_linalg_hard_056_dot_norm_chunked_inputs_preserve_laziness() -> None:
    ...
    right_chunked = AnalysisObject.from_data(
        right.unsafe_data.chunk({"sample": 1}),
        sequence_dim="sample",
        batch_dims=("trial",),
        core_dims=("axis",),
        validate=True,
    ...
    assert out_dot.unsafe_data["datavar"].chunks is not None
    assert out_norm.unsafe_data["datavar"].chunks is not None
